...
chooseNecessaryData(){this.dataForTable=[...JSON.parse(sessionStorage.getItem('data'))];let fullDataForTable=[];for(let i=0;i<this.dataForTable.length;i++){fullDataForTable.push({article:this.dataForTable[i].Article,articleEPC:this.dataForTable[i].ArticleEPC,name:this.dataForTable[i].Name,available:this.dataForTable[i].Available,sklad2:this.dataForTable[i].Sklad2,});}
return fullDataForTable;}
createTable(){let table=document.createElement('table');table.classList.add('copy-table-info');let tableHead=document.createElement('thead');let headTr=document.createElement('tr');let articleTd=document.createElement('td');articleTd.append('Артикул');let articleEpcTd=document.createElement('td');articleEpcTd.append('Артикул ЕПЦ');let nameTd=document.createElement('td');nameTd.append('Наименование');let availableMainTd=document.createElement('td');availableMainTd.append('Остаток');let availableSkl2Td=document.createElement('td');availableSkl2Td.append('Остаток (склад 2)');headTr.append(articleTd);headTr.append(articleEpcTd);headTr.append(nameTd);headTr.append(availableMainTd);headTr.append(availableSkl2Td);tableHead.append(headTr);table.append(tableHead);table.append(this.createTableBody());this.tableWindowData.innerHTML='';this.tableWindowData.append(table);}
createTableBody(){let data=this.chooseNecessaryData();data.sort((a,b)=>parseInt(b.available)-parseInt(a.available));let tableBody=document.createElement('tbody');for(let i=0;i<data.length-1;i++){let tr=document.createElement('tr');let articleTd=document.createElement('td');articleTd.append(data[i].article);let articleEpcTd=document.createElement('td');articleEpcTd.append(data[i].articleEPC);let nameTd=document.createElement('td');nameTd.append(data[i].name);let availableTd=document.createElement('td');availableTd.append(data[i].available);let availableSkl2Td=document.createElement('td');availableSkl2Td.append(data[i].sklad2);tr.append(articleTd);tr.append(articleEpcTd);tr.append(nameTd);tr.append(availableTd);tr.append(availableSkl2Td);tableBody.append(tr);}
return tableBody;}
