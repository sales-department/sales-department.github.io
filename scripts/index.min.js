...
getGoods(){this.disabledBtns();this.mainSectionBody.style.boxShadow='none';$.ajax({url:"https://script.google.com/macros/s/AKfycbwtGbZ-ARvdVMQLgvlJy83ahEMaQPubqWTk800SZqn5u7XwsUmnrDDnkyShTd2ubPlQ/exec",method:"get",dataType:"json",success:(data)=>{setTimeout(()=>{this.elementSection.innerHTML='';sessionStorage.setItem('data',JSON.stringify(data.goods));sessionStorage.setItem('current','fifty');let response=data.goods.reverse();let temporaryGoods=[];for(let i=19;i>=0;i--){temporaryGoods.push(response[i])}
