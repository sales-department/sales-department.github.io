...
this.weatherImg.src=icon;this.weather.style.display='flex';},error:(err)=>{console.log(err.message)},});$.ajax({url:"https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?json",method:"GET",crossDomain:true,dataType:"json",success:(data)=>{let usd=data.filter(f=>f.txt=='Долар США');let resultUsdInfo=usd[0].rate;this.currencyInfo.textContent=`${resultUsdInfo} (НБУ)`;},error:(err)=>{console.log(err.message);},});}}
