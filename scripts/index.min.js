...
module.exports=IndexCharts;},{}],9:[function(require,module,exports){class Login{constructor(){this.body=document.querySelector('body');this.loginWindow=document.querySelector('.log__window');this.loginWindowInput=document.querySelector('#password');this.loginWindowAccept=document.querySelector('.window__login-accept-btn');this.loginWindowReject=document.querySelector('.window__login-reject-btn');this.loginBtn=document.querySelector('.enter-cart-btn');this.AwayBtn=document.querySelector('.back-cart-btn');this.loadReservesBtn=document.querySelector('.load-btn');this.confData=`${Math.sqrt(4)}${Math.sqrt(9)}`+new Date().getFullYear().toString().slice(2);this.init();}
init(){let checkLogin=localStorage.getItem('auth');if(checkLogin=='0'||checkLogin==null){this.loginBtn.style.display='block';this.AwayBtn.style.display='none';}else if(checkLogin=='1'){this.loadReservesBtn.classList.remove('disabled');this.loginBtn.style.display='none';this.AwayBtn.style.display='block';}
...
accept(input){if(input.trim()==this.confData){Swal.fire({icon:"success",title:"Авторизация успешна!",showConfirmButton:false,timer:1500});setTimeout(()=>{localStorage.setItem('auth','1');this.loginBtn.style.display='none';this.AwayBtn.style.display='block';this.loginWindow.style.opacity='0';this.body.style.overflow='';setTimeout(()=>{this.loginWindow.style.display='none';this.loginWindowInput.value='';M.updateTextFields();this.loadReservesBtn.classList.remove('disabled');},200);},1500);}else if(input.trim()!=this.confData){this.loginWindowInput.value='';M.updateTextFields();Swal.fire({icon:"error",title:"Введен неверный код!",showConfirmButton:false,timer:1500});return}}
...
module.exports=Login;},{}],10:[function(require,module,exports){let IndexCharts=require('./index-charts');let IndexChartsNew=require('./index-charts-new');let IndexChartsAvailbl=require('./index-charts-availbl');let IndexChartsReserved=require('./index-charts-reserved');let Login=require('./login');let ScalePhoto=require('./scale-photo');let UpdateData=require('./updated-count');class RenderGoods{constructor(){this.container=document.querySelector('.main-section__body-container');this.mainSectionBody=document.querySelector('.main-section__body');this.elementSection=document.querySelector('.main-section__body-data');this.updata=document.querySelector('.updata');this.fullInfo=document.querySelector('.main-section__body-full-info');this.loader=document.querySelector('.main-section__body-loader');this.notFound=document.querySelector('.main-section__body-not-found');this.searchBtn=document.querySelector('.search-btn');this.filterBtn=document.querySelector('.filter-btn');this.newsBtn=document.querySelector('.news-btn');this.clearBtn=document.querySelector('.clear-btn');this.getAllBtn=document.querySelector('.get-all-btn');this.logInBtn=document.querySelector('.enter-cart-btn');this.logOutBtn=document.querySelector('.back-cart-btn');this.showCalcBnt=document.querySelector('.calculate-btn');this.loadBtn=document.querySelector('.load-btn');this.sampleLoader=document.querySelector('.loader');this.updatedInfo=document.querySelector('.updated-info');this.reservesAll=[];}
init(){let date=new Date();let result=date.toLocaleString();let dateInfo=result.slice(0,10);let timeInfo=result.slice(12);let checkUpdate=localStorage.getItem('update');if(checkUpdate==1){let str=`Обновлено ${dateInfo} в ${timeInfo}`;this.updatedInfo.append(str);this.updatedInfo.style.display='block';localStorage.setItem('update','0');}
$.ajax({url:"https://script.google.com/macros/s/AKfycbxbKaaRcEABRWVU3lvb77XeHfrYAyoYlo0PaObwkF5QM_O_CNw4QBaChM1Wm9LYhziz/exec",method:"get",dataType:"json",success:(data)=>{sessionStorage.setItem('reserves',JSON.stringify(data.reservs));this.reservesAll=[...data.reservs];this.getAllBtn.addEventListener('click',()=>this.getAllGoods());let res=data.reservs.reduce(function(sum,current){return sum+parseInt(current.CountR);},0);sessionStorage.setItem('reservsCount',res);this.getGoods();},error:(err)=>{console.log(`Reserves: ${err}`);},});}
getGoods(){this.disabledBtns();this.mainSectionBody.style.boxShadow='none';$.ajax({url:"https://script.google.com/macros/s/AKfycbxyqS-1vQQtAJGPfkZhYRtbEg1UBo-LCUb2amVidRYDI7Er85VYGEQz_b9HEd29fP7g/exec",method:"get",dataType:"json",success:(data)=>{setTimeout(()=>{this.elementSection.innerHTML='';sessionStorage.setItem('data',JSON.stringify(data.goods));sessionStorage.setItem('current','fifty');let response=data.goods.reverse();let temporaryGoods=[];for(let i=19;i>=0;i--){temporaryGoods.push(response[i])}
let showGoods=temporaryGoods.sort((a,b)=>a.Name>b.Name?1:-1);this.elementSection.append(...showGoods.map(e=>this.createElements(e)));this.loader.style.display='none';this.elementSection.style.display='block';this.updata.style.display='block';let good=data.goods.reduce(function(sum,current){return sum+parseInt(current.Available);},0);sessionStorage.setItem('goodsCount',good);this.removeDisabledBtns();let userStatus=sessionStorage.getItem('user');if(userStatus!='stanislav'){this.logInBtn.classList.remove('disabled');}
setTimeout(()=>{let chartAll=document.querySelector('.chart-all');let chartNew=document.querySelector('.chart-new');let search=document.querySelector('.main-section__body-search');this.mainSectionBody.style.boxShadow='0 0 3px rgba(0, 0, 0, 0.3)';new Login();search.style.display='block';chartAll.style.display='block';chartNew.style.display='flex';new IndexCharts();new IndexChartsNew();new IndexChartsAvailbl();new IndexChartsReserved();new ScalePhoto();new UpdateData();},100)},500);},error:(err)=>{this.updata.style.display='block';this.elementSection.innerHTML='';this.elementSection.append(`Ошибка загрузки... Обновите страницу!`);this.elementSection.style.textAlign='center';this.elementSection.style.color='red';this.elementSection.style.fontWeight='bold';this.elementSection.style.display='flex';this.elementSection.style.justifyContent='center';this.elementSection.style.alignItems='center';this.elementSection.style.fontSize='25px';this.loader.style.display='none';this.removeDisabledBtns();},});}
...
let filterElemArr=this.reservesAll.filter(f=>f.CodeR==data.Code);let reservSummaryCount=filterElemArr.reduce(function(sum,current){return sum+parseInt(current.CountR);},0);let reserved=document.createElement('div');reserved.classList.add('item__reserved');reserved.innerText=reservSummaryCount;let available=document.createElement('div');available.classList.add('item__available');available.innerText=data.Available;if(parseInt(data.Available)>0){available.classList.add('av_green');}else if(parseInt(data.Available)==0){available.classList.add('av_orange');}
element.append(reserved);element.append(available);return element;}
disabledBtns(){this.searchBtn.classList.add('disabled');this.filterBtn.classList.add('disabled');this.newsBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.getAllBtn.classList.add('disabled');this.logInBtn.classList.add('disabled');this.logOutBtn.classList.add('disabled');this.showCalcBnt.classList.add('disabled');this.loadBtn.classList.add('disabled');}
removeDisabledBtns(){this.searchBtn.classList.remove('disabled');this.filterBtn.classList.remove('disabled');this.newsBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.getAllBtn.classList.remove('disabled');this.logInBtn.classList.remove('disabled');this.logOutBtn.classList.remove('disabled');this.showCalcBnt.classList.remove('disabled');let checkLogin=localStorage.getItem('auth');if(checkLogin=='0'||checkLogin==null){return}else if(checkLogin=='1'){this.loadBtn.classList.remove('disabled');}}}
module.exports=RenderGoods;},{"./index-charts":8,"./index-charts-availbl":5,"./index-charts-new":6,"./index-charts-reserved":7,"./login":9,"./scale-photo":11,"./updated-count":15}],11:[function(require,module,exports){class ScalePhoto{constructor(){this.body=document.querySelector('body');this.scalePhotoContainer=document.querySelector('.scale-photo-container');this.scalePhoto=document.querySelector('.scale-img');this.scalePhotoClose=document.querySelector('.scale-photo-close');this.scalePhotoDownload=document.querySelector('.dwnld-img');this.photoAttribute=null;this.init();}
...
module.exports=ScalePhoto;},{}],12:[function(require,module,exports){class Searching{constructor(){this.container=document.querySelector('.main-section__body-container');this.elementSection=document.querySelector('.main-section__body-data');this.updata=document.querySelector('.updata');this.searchBtn=document.querySelector('.search-btn');this.filterBtn=document.querySelector('.filter-btn');this.newsBtn=document.querySelector('.news-btn');this.clearBtn=document.querySelector('.clear-btn');this.getAllBtn=document.querySelector('.get-all-btn');this.loadBtn=document.querySelector('.load-btn');this.sampleLoader=document.querySelector('.loader');this.loader=document.querySelector('.main-section__body-loader');this.fullInfo=document.querySelector('.main-section__body-full-info');this.notFound=document.querySelector('.main-section__body-not-found');this.search=document.querySelector('.search-input');this.findTempArr=[];this.searchArr=[];this.findCount=null;this.singleElem=null;this.reservesAll=[];this.findTextData=[];this.init();}
...
disabledBtns(){this.container.style.backgroundImage='linear-gradient(to bottom, snow, snow)';this.searchBtn.classList.add('disabled');this.filterBtn.classList.add('disabled');this.newsBtn.classList.add('disabled');this.clearBtn.classList.add('disabled');this.getAllBtn.classList.add('disabled');this.loadBtn.classList.add('disabled');}
removeDisabledBtns(){this.container.style.backgroundImage='linear-gradient(to bottom, #f3f4fa, #e9edff)';this.searchBtn.classList.remove('disabled');this.filterBtn.classList.remove('disabled');this.newsBtn.classList.remove('disabled');this.clearBtn.classList.remove('disabled');this.getAllBtn.classList.remove('disabled');let checkLogin=localStorage.getItem('auth');if(checkLogin=='0'||checkLogin==null){return}else if(checkLogin=='1'){this.loadBtn.classList.remove('disabled');}}}
...
module.exports=ShowAsside;},{}],14:[function(require,module,exports){class TableCount{constructor(){this.dataForTable=[];this.bodySelector=document.querySelector('body');this.loadBtn=document.querySelector('.load-btn');this.tableWindow=document.querySelector('.table__window');this.tableWindowData=document.querySelector('.table__window-data');this.tableWindowClose=document.querySelector('.table__window-close');this.tableWindowCopy=document.querySelector('.table__data-dowload');this.mainSection=document.querySelector('.main-section');this.searchInput=document.querySelector('.search-input');this.searchInputData=[];this.init();}
init(){this.loadBtn.addEventListener('click',()=>this.showWindow());this.tableWindowClose.addEventListener('click',()=>this.closeWindow());this.tableWindowCopy.addEventListener('click',()=>this.copyTable());}
showWindow(){this.searchInputData=[];this.loadBtn.classList.add('disabled');this.bodySelector.style.height='100vh';this.bodySelector.style.overflow='hidden';this.mainSection.style.height='100vh';this.mainSection.style.overflow='hidden';this.tableWindow.style.display='flex';if(this.searchInput.value.trim()!==''){let tempArr=this.searchInput.value.trim().split(',');for(let i=0;i<tempArr.length;i++){if(tempArr[i].trim()!==''){this.searchInputData.push(tempArr[i].trim())}}
this.searchInput.value='';}
this.createTable();}
closeWindow(){this.bodySelector.style.height='100%';this.bodySelector.style.overflow='';this.mainSection.style.height='100%';this.mainSection.style.overflow='';this.tableWindow.style.display='none';this.loadBtn.classList.remove('disabled');}
chooseNecessaryData(){this.reservTable=[...JSON.parse(sessionStorage.getItem('reserves'))];let tempArr=[];let uniqueReservesCodesArr=[];for(let i=0;i<this.reservTable.length;i++){tempArr.push(this.reservTable[i].CodeR);}
uniqueReservesCodesArr=[...new Set(tempArr)];let reservSummaryCount=[];let count=null;for(let i=0;i<uniqueReservesCodesArr.length;i++){let filterElemArr=this.reservTable.filter(f=>f.CodeR==uniqueReservesCodesArr[i]);count=filterElemArr.reduce(function(sum,current){return sum+parseInt(current.CountR);},0)
reservSummaryCount.push({codeR:uniqueReservesCodesArr[i],countR:count});}
let fullDataForTable=[];this.dataForTable=[...JSON.parse(sessionStorage.getItem('data'))];for(let i=0;i<this.dataForTable.length;i++){this.dataForTable[i].Reserved=0}
for(let i=0;i<reservSummaryCount.length;i++){for(let j=0;j<this.dataForTable.length;j++){if(reservSummaryCount[i].codeR==this.dataForTable[j].Code){this.dataForTable[j].Reserved=reservSummaryCount[i].countR}}}
for(let i=0;i<this.dataForTable.length;i++){fullDataForTable.push({code:this.dataForTable[i].Code,article:this.dataForTable[i].Article,articleEPC:this.dataForTable[i].ArticleEPC,shtrihCode:this.dataForTable[i].ShtrihCode,uktz:this.dataForTable[i].UktzCode,name:this.dataForTable[i].Name,price:this.dataForTable[i].Price,available:this.dataForTable[i].Available,sklad2:this.dataForTable[i].Sklad2,reserv:this.dataForTable[i].Reserved,});}
if(this.searchInputData.length<1){return fullDataForTable;}else{let filteredData=[];for(let i=0;i<this.searchInputData.length;i++){for(let j=0;j<fullDataForTable.length;j++){if(fullDataForTable[j].code==this.searchInputData[i]){filteredData.push(fullDataForTable[j]);}}}
return filteredData;}}
createTable(){let table=document.createElement('table');table.classList.add('copy-table-info');let tableHead=document.createElement('thead');let headTr=document.createElement('tr');let codeTd=document.createElement('td');codeTd.append('Код');let articleTd=document.createElement('td');articleTd.append('Артикул');let articleEpcTd=document.createElement('td');articleEpcTd.append('Артикул ЕПЦ');let shtrihCodeTd=document.createElement('td');shtrihCodeTd.append('Штрихкод');let uktzTd=document.createElement('td');uktzTd.append('УКТЗ');let nameTd=document.createElement('td');nameTd.append('Наименование');let priceTd=document.createElement('td');priceTd.append('Цена (база)');let availableMainTd=document.createElement('td');availableMainTd.append('Остаток');let availableSkl2Td=document.createElement('td');availableSkl2Td.append('Остаток (ДПМ)');let reservTd=document.createElement('td');reservTd.append('Резерв');headTr.append(codeTd);headTr.append(articleTd);headTr.append(articleEpcTd);headTr.append(shtrihCodeTd);headTr.append(uktzTd);headTr.append(nameTd);headTr.append(priceTd);headTr.append(availableMainTd);headTr.append(availableSkl2Td);headTr.append(reservTd);tableHead.append(headTr);table.append(tableHead);table.append(this.createTableBody());this.tableWindowData.innerHTML='';this.tableWindowData.append(table);}
createTableBody(){let data=this.chooseNecessaryData();data.sort((a,b)=>parseInt(b.available)-parseInt(a.available));let tableBody=document.createElement('tbody');for(let i=0;i<data.length;i++){let tr=document.createElement('tr');let codeTd=document.createElement('td');codeTd.append(data[i].code);let articleTd=document.createElement('td');articleTd.append(data[i].article);let articleEpcTd=document.createElement('td');articleEpcTd.append(data[i].articleEPC);let shtrihCodeTd=document.createElement('td');shtrihCodeTd.append(data[i].shtrihCode);let uktzTd=document.createElement('td');uktzTd.append(data[i].uktz);let nameTd=document.createElement('td');nameTd.append(data[i].name);let priceTd=document.createElement('td');priceTd.append(data[i].price);let availableTd=document.createElement('td');availableTd.append(data[i].available);let availableSkl2Td=document.createElement('td');availableSkl2Td.append(data[i].sklad2);let reservTd=document.createElement('td');reservTd.append(data[i].reserv);tr.append(codeTd);tr.append(articleTd);tr.append(articleEpcTd);tr.append(shtrihCodeTd);tr.append(uktzTd);tr.append(nameTd);tr.append(priceTd);tr.append(availableTd);tr.append(availableSkl2Td);tr.append(reservTd);tableBody.append(tr);}
return tableBody;}
copyTable(){navigator.clipboard.writeText(document.querySelector(".copy-table-info").innerText).then(function(){M.toast({html:`Скопировано в буфер обмена`,displayLength:1500,classes:'toast-green'});}).catch(function(error){M.toast({html:`Ошибка! ${error}`,displayLength:1500,classes:'toast-red'});});}}
module.exports=TableCount;},{}],15:[function(require,module,exports){class UpdatedCount{constructor(){this.init();}
init(){let allGoodsCount=sessionStorage.getItem('goodsCount');let allGoodsReserves=sessionStorage.getItem('reservsCount');let allGoodsCountNew=null;let allGoodsReservesNew=null;setInterval(()=>{getInfoNew();},600000);async function getInfoNew(){let goods;let reserves;await $.ajax({url:"https://script.google.com/macros/s/AKfycbxyqS-1vQQtAJGPfkZhYRtbEg1UBo-LCUb2amVidRYDI7Er85VYGEQz_b9HEd29fP7g/exec",method:"get",dataType:"json",success:(data)=>{let response=data.goods;showInfo();function showInfo(){allGoodsCountNew=response.reduce(function(sum,current){return sum+parseInt(current.Available);},0);}
goods=Math.abs(parseInt(allGoodsCount)-parseInt(allGoodsCountNew));},error:(err)=>{console.log(`Reserves: ${err}`);},});await $.ajax({url:"https://script.google.com/macros/s/AKfycbxbKaaRcEABRWVU3lvb77XeHfrYAyoYlo0PaObwkF5QM_O_CNw4QBaChM1Wm9LYhziz/exec",method:"get",dataType:"json",success:(data)=>{let response=data.reservs;showInfo();function showInfo(){allGoodsReservesNew=response.reduce(function(sum,current){return sum+parseInt(current.CountR);},0);}
reserves=Math.abs(parseInt(allGoodsReserves)-parseInt(allGoodsReservesNew));},error:(err)=>{console.log(`Reserves: ${err}`);},});await calculateDifferenceCount(goods,reserves)}
function calculateDifferenceCount(good,res){if(good!=0||res!=0){localStorage.setItem('update','1');location.reload();}}}}
module.exports=UpdatedCount;},{}],16:[function(require,module,exports){class Weather{constructor(){this.weather=document.querySelector('.weather');this.weatherCity=document.querySelector('.weather__city');this.weatherTemp=document.querySelector('.weather__temperature');this.weatherImg=document.querySelector('.weather__img');this.currencyInfo=document.querySelector('.currency-info');this.showWeather();setInterval(()=>{this.showWeather();},1800000)}
showWeather(){$.ajax({url:"https://api.weatherapi.com/v1/current.json?key=5f6ae58e18e644888bf182855242904&q=Днепропетровск",method:"get",crossDomain:true,dataType:"json",success:(data)=>{let location=data.location.name;let temperature=data.current.temp_c;let icon=data.current.condition.icon;this.weatherCity.textContent=location.slice(0,5);this.weatherTemp.textContent=`${temperature}°C`;if(parseFloat(temperature)>=35){this.weatherTemp.style.color='orangered';}else if(parseFloat(temperature)<0){this.weatherTemp.style.color='dodgerblue';}
this.weatherImg.src=icon;setTimeout(()=>{this.weather.style.display='flex';},1500);},error:(err)=>{console.log(err.message)},});$.ajax({url:"https://bank.gov.ua/NBUStatService/v1/statdirectory/exchange?json",method:"GET",crossDomain:true,dataType:"json",success:(data)=>{let usd=data.filter(f=>f.txt=='Долар США');let resultUsdInfo=usd[0].rate;this.currencyInfo.textContent=resultUsdInfo;},error:(err)=>{console.log(err.message);},});}}
