...
module.exports=IndexCharts;},{}],9:[function(require,module,exports){class Login{constructor(){this.body=document.querySelector('body');this.loginWindow=document.querySelector('.login-window');this.loginWindowInput=document.querySelector('#enter-code');this.loginWindowAccept=document.querySelector('.e-btn');this.loginWindowReject=document.querySelector('.r-btn');this.loginBtn=document.querySelector('.enter-cart-btn');this.AwayBtn=document.querySelector('.back-cart-btn');this.confData='2'+'3'+new Date().getFullYear().toString().slice(2);this.init();}
...
this.loginBtn.addEventListener('click',()=>this.logIn());this.AwayBtn.addEventListener('click',()=>this.logOut());}
logIn(){this.body.style.overflow='hidden';this.loginWindow.style.display='flex';this.loginWindowAccept.addEventListener('click',()=>this.accept(this.loginWindowInput.value));this.loginWindowReject.addEventListener('click',()=>this.reject());}
logOut(){let confirmExit=confirm("Вы уверены?");if(confirmExit){localStorage.setItem('auth','0');location.reload();}else{return}}
accept(input){if(input.trim()==this.confData){localStorage.setItem('auth','1');this.body.style.overflow='';this.loginWindow.style.display='none';this.loginWindowInput.value='';M.updateTextFields();this.loginBtn.style.display='none';this.AwayBtn.style.display='block';}else if(input.trim()!=this.confData){this.loginWindowInput.value='';M.updateTextFields();alert('Введен неверный код!');return}}
reject(){this.body.style.overflow='';this.loginWindow.style.display='none';this.loginWindowInput.value='';M.updateTextFields();}}
...
this.removeDisabledBtns();this.search.value='';let checkLogin=localStorage.getItem('auth');if(checkLogin=='1')$('.collapsible').collapsible();},200);}}
...
this.removeDisabledBtns();this.search.value='';let checkLogin=localStorage.getItem('auth');if(checkLogin=='1')$('.collapsible').collapsible();},100);}
createElements(data){let element=document.createElement('div');element.classList.add('item');element.dataset.index=data.Id;let logo=document.createElement('div');logo.classList.add('item__logo');let img=document.createElement('img');let checkArticle=data.Article;let result;let findSlash=checkArticle.indexOf('/');if(findSlash!=-1){let check=checkArticle.split('');let final;let find=check.indexOf('/');check[find]='-';final=[...check];result=final.join('');checkArticle=result;}
img.setAttribute('src',`./images/photo-all/images/${checkArticle}.webp`);logo.append(img);element.append(logo)
let itemCode=document.createElement('div');itemCode.classList.add('item__code');let code=document.createElement('div');code.classList.add('code');code.append(data.Code);itemCode.append(code);element.append(itemCode);let itemArticle=document.createElement('div');itemArticle.classList.add('item__article');let article=document.createElement('div');article.classList.add('article');article.append(data.Article);itemArticle.append(article);element.append(itemArticle);let itemName=document.createElement('div');itemName.classList.add('item__title');let name=document.createElement('div');name.classList.add('title');name.append(data.Name);itemName.append(name);element.append(itemName);let status=document.createElement('div');status.classList.add('item__status');if(data.Status==='new'){element.append(status);}
return element}
