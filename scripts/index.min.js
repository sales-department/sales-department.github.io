...
module.exports=ShowAsside;},{}],14:[function(require,module,exports){class TableCount{constructor(){this.dataForTable=[...JSON.parse(sessionStorage.getItem('data'))];this.bodySelector=document.querySelector('body');this.loadBtn=document.querySelector('.load-btn');this.tableWindow=document.querySelector('.table__window');this.tableWindowData=document.querySelector('.table__window-data');this.tableWindowClose=document.querySelector('.table__window-close');this.tableWindowCopy=document.querySelector('.table__data-dowload');this.mainSection=document.querySelector('.main-section');this.init();}
...
chooseNecessaryData(){let fullDataForTable=[];for(let i=0;i<this.dataForTable.length;i++){fullDataForTable.push({article:this.dataForTable[i].Article,articleEPC:this.dataForTable[i].ArticleEPC,name:this.dataForTable[i].Name,available:this.dataForTable[i].Available,sklad2:this.dataForTable[i].Sklad2,});}
return fullDataForTable;}
createTable(){let table=document.createElement('table');table.classList.add('copy-table-info');let caption=document.createElement('caption');caption.append('Информация по остаткам товара');let tableHead=document.createElement('thead');let headTr=document.createElement('tr');let articleTd=document.createElement('td');articleTd.append('Артикул');let articleEpcTd=document.createElement('td');articleEpcTd.append('Артикул ЕПЦ');let nameTd=document.createElement('td');nameTd.append('Наименование');let availableMainTd=document.createElement('td');availableMainTd.append('Остаток (осн)');let availableSkl2Td=document.createElement('td');availableSkl2Td.append('Остаток (скл2)');headTr.append(articleTd);headTr.append(articleEpcTd);headTr.append(nameTd);headTr.append(availableMainTd);headTr.append(availableSkl2Td);tableHead.append(headTr);table.append(caption);table.append(tableHead);table.append(this.createTableElements());this.tableWindowData.innerHTML='';this.tableWindowData.append(table);}
createTableElements(){let data=this.chooseNecessaryData();data.sort((a,b)=>parseInt(b.available)-parseInt(a.available));let tableBody=document.createElement('tbody');for(let i=0;i<data.length-1;i++){let tr=document.createElement('tr');let articleTd=document.createElement('td');articleTd.append(data[i].article);let articleEpcTd=document.createElement('td');articleEpcTd.append(data[i].articleEPC);let nameTd=document.createElement('td');nameTd.append(data[i].name);let availableTd=document.createElement('td');availableTd.append(data[i].available);let availableSkl2Td=document.createElement('td');availableSkl2Td.append(data[i].sklad2);tr.append(articleTd);tr.append(articleEpcTd);tr.append(nameTd);tr.append(availableTd);tr.append(availableSkl2Td);tableBody.append(tr);}
return tableBody;}
copyTable(){navigator.clipboard.writeText(document.querySelector(".copy-table-info").innerText).then(function(){M.toast({html:`Данные скопированы`,displayLength:1500,classes:'toast-green'});}).catch(function(error){M.toast({html:`Ошибка! ${error}`,displayLength:1500,classes:'toast-red'});});}}
